%%% File encoding: UTF-8
%%% äöüÄÖÜß  <-- keine deutschen Umlaute hier? UTF-faehigen Editor verwenden!

%%% Magic Comments zum Setzen der korrekten Parameter in kompatiblen IDEs
% !TeX encoding = utf8
% !TeX program = pdflatex 
% !TeX spellcheck = de_DE
% !BIB program = biber

\documentclass[bachelor,german]{hgbthesis}
% Zulässige Optionen in [..]: 
%   Typ der Arbeit: diploma, master (default), bachelor, internship 
%   Hauptsprache: german (default), english
%%%----------------------------------------------------------

\RequirePackage[utf8]{inputenc}		% bei der Verw. von lualatex oder xelatex entfernen!

\graphicspath{{images/}}    % Verzeichnis mit Bildern und Grafiken
\logofile{logo}				% Logo-Datei = images/logo.pdf (\logofile{}, wenn kein Logo gewünscht)
\bibliography{references}  	% Biblatex-Literaturdatei (references.bib)

%%%----------------------------------------------------------
% Angaben für die Titelei (Titelseite, Erklärung etc.)
%%%----------------------------------------------------------

%%% Einträge für ALLE Arbeiten: -----------------------------
\title{Industrielle Cloud Applikationen in Industrie 4.0 mit Siemens MindSphere}
\author{Elisabeth \ Egger}
\programname{??? }
\placeofstudy{Hagenberg}
\dateofsubmission{2018}{02}{28}	% {YYYY}{MM}{DD}

%%% Zusätzlich für eine Bachelorarbeit: ---------------------
\thesisnumber{S1510307005-A}   % Stud-ID, z.B. 1310238045-A  
% (A = 1. Bachelorarbeit)
\semester{Wintersemester 2017/2018} 
\coursetitle{???} 
\advisor{Susanne Schaller, \ MMSc}

%%% Restriktive Lizenformel anstatt CC (nur für Typ master) -
%\strictlicense

%%%----------------------------------------------------------
\begin{document}
%%%----------------------------------------------------------

%%%----------------------------------------------------------
\frontmatter                    % Titelei (röm. Seitenzahlen)
%%%----------------------------------------------------------

\maketitle
\tableofcontents

%\include{thesis_DE/front/vorwort} % Optional. Ggf. weglassen
\include{bachelorthesis_ElisabethEgger/front/kurzfassung}		
\include{bachelorthesis_ElisabethEgger/front/abstract}			

%%%----------------------------------------------------------
\mainmatter          % Hauptteil (ab hier arab. Seitenzahlen)
%%%----------------------------------------------------------

\include{bachelorthesis_ElisabethEgger/chapters/einleitung}
\include{bachelorthesis_ElisabethEgger/chapters/industrie40}
\include{bachelorthesis_ElisabethEgger/chapters/iot}
\include{bachelorthesis_ElisabethEgger/chapters/mindsphere}
\include{bachelorthesis_ElisabethEgger/chapters/aufgabenstellung}
\include{bachelorthesis_ElisabethEgger/chapters/implementierung}
\include{bachelorthesis_ElisabethEgger/chapters/zusammenfassung}



%%%----------------------------------------------------------
\appendix                                            % Anhang 
%%%----------------------------------------------------------

%\include{thesis_DE/back/anhang_a}	% Technische Ergänzungen
%\include{thesis_DE/back/anhang_b}	% Inhalt der CD-ROM/DVD
%\include{thesis_DE/back/anhang_c}	% Chronologische Liste der Änderungen
%\include{thesis_DE/back/anhang_d}	% Quelltext dieses Dokuments

%%%----------------------------------------------------------
\MakeBibliography                        % Quellenverzeichnis
%%%----------------------------------------------------------

%%% Messbox zur Druckkontrolle ------------------------------
%\include{thesis_DE/back/messbox}

%%%----------------------------------------------------------
\end{document}
%%%----------------------------------------------------------